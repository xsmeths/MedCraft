...
public class BandageHandler
{
  ...
  public BandageHandler(final Player p) {
    if (bandagingPlayers.containsKey(p)) {
      ((BukkitRunnable)bandagingPlayers.get(p)).cancel();
      bandagingPlayers.remove(p);
      p.getInventory().addItem(new ItemStack[] { BandageItemLoader.getBandageItem() });
    }
    ...
    ((BukkitRunnable)bandagingPlayers.get(p)).runTaskTimer(MedCraft.getPlugin(), 0L, 1L);
  }
